...
- Matching rule (misattribution-safe): burst timestamps are ambiguous, so the connector returns up to 4 candidate contents whose capture ts (contents field 11.17.5) is within [-10s lead, +30s trail] of the message's ServerCreatedAt, sorted by closeness; the bridge attaches every candidate key (`MediaAttachment.AltKeys`) and `finalizeDownloadedMedia` tries each until decryption+validation succeeds. Only the correct key decrypts, so misattribution is impossible end to end. Observed skews: Lorelei ~0-10s, Gabi ~11s.
...
- Operator resyncs bypass the EEL rate cap (`snapapi.WithEELThrottleBypass` context in resync-all) — run `bridge_admin.py resync <chat-id>` to hydrate a chat's snaps on demand.
...
- LIVE VERIFIED (chat Gabi 2caa1464): resync hydrated 1517/1519/1521 → "uploaded Snapchat media" ×3 in portal !CdUGA8sIO2ZM9W4pHeZ3. Remaining placeholders (1502-1516, 1518, 1520, 1522) are snaps whose contents are outside the app page's current window — they stay retryable (2-min backoff + resyncs) and need the pinned syncServerConversation to fetch deeper history (pagination remains forbidden).
- Bridge pid 52528. Bridge-side binary rebuilt; message_fetch_limit stays at 70.
